...
import { by_email } from '$lib/server/profile';
...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	const hn = p.e ? ((await by_email(env, String(p.e)))?.hn ?? '') : '';
	delete p.e;
	...
	if (!own) return { p, own, can_msg, hn, rel, asks: [] };
	...
	return { p, own, can_msg, hn, rel, asks };
};
