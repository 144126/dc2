...
export const momentum = (p: P): number =>
	(p.hm ? 10 : 0) +
	(p.hv === 'v' ? 5 : 0) +
	(p.m === 'y' ? 20 : 0) +
	log_points(p.a, 4, 20) +
...
export const by_momentum = (a: P, b: P): number =>
	momentum(b) - momentum(a) || (a.n ?? '').localeCompare(b.n ?? '');

export const headline = (p: P): { v: string; l: string; hv: string } | null => {
	if (p.hm) return { v: p.hm, l: p.hl || 'headline metric', hv: p.hv || 's' };
	if (p.k) return { v: p.k, l: 'proudest metric', hv: 's' };
	if (p.q) return { v: p.q, l: 'users', hv: 's' };
	if (p.m === 'y' && p.a) return { v: p.a, l: 'monthly revenue (₦)', hv: 's' };
	return null;
};